...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private double r = 5;  //promien
    private double kat = 0; //k¹t
    private float x=0,z=0; //wspolzedne
	...
	private void draw(GL2 gl2) {
		...
		if (objectNumber == 1) Korkociag(gl2,0.7,24); //12 obrotow
		
		else if (objectNumber == 2) Piramida(gl2); //12 wierzcholkow
		
		else if (objectNumber == 3) PiramidaFunkcja(gl2,12);
	 ...
	 */
	public void DrawCirclre(double x, double y, double z, double r)
	{
		// nic tu nie ma
	}
	...
	public void Polygon(GL2 gl2,int n){
	 ...
	 public void Trojkat(GL2 gl2){
	        ...
	        gl2.glVertex3f( 1.6f, -1f, 0 );
	        gl2.glVertex3f( 0f, 7f, -4.9f );
	        gl2.glVertex3f( -1.1f, -1f, 0 );
	        gl2.glEnd();

	    }
	
	public void Piramida(GL2 gl2)
	{
		gl2.glBegin(GL2.GL_POLYGON);
        gl2.glColor3f( 0f, 0.4f, 1f );
		Polygon(gl2,12);
		
		gl2.glMatrixPushEXT(1);
        gl2.glColor3f( 1f, 1f, 0 );
        gl2.glTranslated(4.6,0,1.5);
        gl2.glRotated(74,0,1,0);
        Trojkat(gl2); //1
        gl2.glColor3f( 1f, 0, 0 );
        gl2.glTranslated(-2.45,0,-0.7);
        gl2.glRotated(-30,0,1,0);
        Trojkat(gl2); //2
        gl2.glColor3f( 1f, 0, 1f );
        gl2.glTranslated(-2.2,0,-0.7f);
        gl2.glRotated(-28,0,1,0);
        Trojkat(gl2); //3
        gl2.glColor3f( 0, 1f, 1f );
        gl2.glTranslated(-2.45,0,-0.85);
        gl2.glRotated(-30,0,1,0);
        Trojkat(gl2); //4
        gl2.glColor3f( 0, 0, 1f );
        gl2.glTranslated(-2.4,0,-0.7);
        gl2.glRotated(-31,0,1,0);
        Trojkat(gl2); //5
        ...
        gl2.glRotated(-29.5,0,1,0);
        Trojkat(gl2); //9
        gl2.glColor3f( 0.9f, 0.2f, 0.3f );
        gl2.glTranslated(-2.5,0,-0.8);
        gl2.glRotated(-30,0,1,0);
        Trojkat(gl2); //10
        gl2.glColor3f( 0.3f, 0.2f, 0.9f );
        gl2.glTranslated(-2.5,0,-0.8);
        gl2.glRotated(-31,0,1,0);
        Trojkat(gl2); //11
        gl2.glColor3f( 0.4f, 0.9f, 0.3f );
        gl2.glTranslated(-2.5,0,-0.8);
        gl2.glRotated(-31,0,1,0);
        Trojkat(gl2); //12
	...
	public void Korkociag(GL2 gl2, double r, int n)
	{
		gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);
        ...
        double zmiana = n * Math.PI / ilosc; //zwiekszenie k¹tu
        ...
        for (int i = 1; i <= ilosc; i++) {
        	gl2.glColor3f(0.658f, 0.678f, 0.678f);
            x = (float) (r * Math.cos(kat));  // liczymy x i y
            z = (float) (r * Math.sin(kat));
            gl2.glVertex3f(x, wysokosc, z); // punkt przesuwamy
            kat = i * zmiana;
            wysokosc+=n/18000f;
        }

        gl2.glEnd();
        gl2.glColor3f(0.741f, 0.368f, 0.196f);
        gl2.glTranslated(0,wysokosc,-3.5);
        glu.gluCylinder(glu.gluNewQuadric(),1,1,7,100,50);   //trzonek
      //glu.gluDisk(glu.gluNewQuadric(), 0, 5, arg3, arg4);
         //dekle od trzonka
      
        gl2.glMatrixPopEXT(1);
        glu.gluDisk(glu.gluNewQuadric(), 0, 1, 100, 1);
	 ...
	 public void PiramidaFunkcja(GL2 gl2,int n){
	        ...
	        for (int i = 1; i <= n; i++) {
	            ...
	            x = (float) (r * Math.cos(kat));
	            z = (float) (r * Math.sin(kat));
	            gl2.glColor3f( 1f, 1.0f, 1f ); 
	            gl2.glVertex3f( x, -1f, z );
	            gl2.glColor3f( 1.0f, 1f, 1f ); 
	            gl2.glVertex3f( 0.0f, 7.0f, 0.0f );

	            kat = i * angleIncrement;

	            gl2.glColor3f( 0.0f, 1f, 1.0f ); 
	            gl2.glVertex3f((float) (r * Math.cos(kat)), -1f,(float) (r * Math.sin(kat)));
	            gl2.glEnd();
	        }
